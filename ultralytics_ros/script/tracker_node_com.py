...
class TrackerNode:
    def __init__(self):
        ...
        self.sub = rospy.Subscriber(image_topic, CompressedImage, self.image_callback, queue_size=1, buff_size=2**24)
    ...
    def image_callback(self, msg):
        ...
        # Decompresses the compressed image data
        np_arr = np.frombuffer(msg.data, np.uint8) # # Converts the compressed image data from ROS message to a NumPy array
        cv_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR) # Decodes the compressed NumPy array into a color image using OpenCV

        header = msg.header
        encoding = msg.format

        # Processes the image using the YOLO model
       ...
       # Creates a copy of the image to overlay the FPS text on
