...
class TrackerNode:
    def __init__(self, image_topic, camera_frame_id, debug_image_topic):
        ...
        # Subscribes to camera information topic for calibration data 
        rospy.Subscriber(f"{image_topic.replace('/color/image_raw', '/color/camera_info')}", CameraInfo, self.camerainfo_callback, queue_size=1)
        
        # Subscribes to depth image topic for raw depth data 
        rospy.Subscriber(f"{image_topic.replace('/color/image_raw', '/depth/image_raw')}", Image, self.depth_callback, queue_size=1)
        
        # The coordinate frame associated with the camera
    ...
    def image_callback(self, msg):
        ...
        if hasattr(results[0], 'keypoints') and results[0].keypoints is not None:  # Checks whether keypoints' information is available in the result
            ...
            for person_idx, bbox in enumerate(bounding_boxes):  
                ...
                self.visualize_stick_figure(new_world_coordinates, person_idx, self.camera_frame_id)
        ...
        if t0 > clear_interval:
            self.visualize_stick_figure(empty_coordinates, 0, self.camera_frame_id)
            
        if t1 > clear_interval:
            self.visualize_stick_figure(empty_coordinates, 1, self.camera_frame_id)
            
        if t2 > clear_interval:
            self.visualize_stick_figure(empty_coordinates, 2, self.camera_frame_id)
    ...
    def visualize_stick_figure(self, new_world_coordinates, person_idx, camera_frame_id):
        
        color = self.get_person_color(person_idx) # Retrieves the color for a specific person
        
        topic = f"{camera_frame_id}_stick_figure_marker_{person_idx}"
       
        marker = Marker() # Creates a new Marker instance
        marker.header.frame_id = camera_frame_id  # Sets the frame ID for the marker's reference frame (coordinate system)
        ...
        color = self.get_person_color(person_idx+1)
...
if __name__ == "__main__":
    rospy.init_node("tracker_node")
        ...
        # Front Camera Node
    node_front = TrackerNode(
        image_topic="/front_camera/color/image_raw",
        camera_frame_id="front_camera_link",
        debug_image_topic="front_camera_debug_image"
    )
    
    # Right Camera Node
    node_right = TrackerNode(
        image_topic="/right_camera/color/image_raw",
        camera_frame_id="right_camera_link",
        debug_image_topic="right_camera_debug_image"
    )
    
    # Back Camera Node
    #node_back = TrackerNode(
        #image_topic="/back_camera/color/image_raw",
        #camera_frame_id="back_camera_link",
        #debug_image_topic="back_camera_debug_image"
    #)
    
    # Left Camera Node
    node_left = TrackerNode(
        image_topic="/left_camera/color/image_raw",
        camera_frame_id="left_camera_link",
        debug_image_topic="left_camera_debug_image"
    )
